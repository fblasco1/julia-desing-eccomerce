...
{% if not cart_page %}
  ...
  <div class="js-visible-on-cart-filled h5 {% if not cart_page %}row no-gutters{% else %}text-right{% endif %} mb-1 {% if cart_page %}text-center-xs{% endif %}" {% if cart.items_count == 0 %}style="display:none;"{% endif %} data-store="cart-subtotal">
    <span {% if not cart_page %}class="col"{% endif %}>
      {{ "Subtotal" | translate }}
      <small class="js-subtotal-shipping-wording" {% if not ((cart.has_shippable_products is defined and cart.has_shippable_products) or show_calculator_on_cart) %}style="display: none"{% endif %}>{{ " (sin envío)" | translate }}</small>
      :
    </span>
...
{% endif %}
    ...
    {% endif %}
      {% if show_cart_fulfillment %}
        <div class="js-fulfillment-info js-allows-non-shippable" {% if cart.has_shippable_products is not defined or not cart.has_shippable_products %}style="display: none"{% endif %}>
    ...
    {% endif %}
              ...
              <div class="js-cart-total-container js-visible-on-cart-filled mb-3 clear-both" {% if cart.items_count == 0 %}style="display:none;"{% endif %} data-store="cart-total">
                <div class="h2 row no-gutters text-primary mb-0 {% if cart_page %}justify-content-end justify-content-md-center{% endif %}">
                  ...
                  <span class="js-cart-total {% if cart.free_shipping is defined and cart.free_shipping.cart_has_free_shipping %}js-free-shipping-achieved{% endif %} {% if cart.shipping_data is defined and cart.shipping_data.selected %}js-cart-saved-shipping{% endif %} col {% if cart_page %}col-md-auto{% endif %} text-right" data-component="cart.total" data-component-value={{ cart.total }}>{{ cart.total | money }}</span>
                </div>
