...
{% if cart.free_shipping is defined %}
    {% set has_free_shipping = cart.free_shipping.cart_has_free_shipping or (cart.free_shipping.min_price_free_shipping is defined and cart.free_shipping.min_price_free_shipping.min_price) %}
{% endif %}
    ...
    <div class="js-free-shipping-minimum-message free-shipping-message text-center text-md-left mb-4 pb-2">
        ...
        <span class="d-inline-block">
            ...
            <span {% if has_product_free_shipping %}style="display: none;"{% else %}class="js-shipping-minimum-label"{% endif %}>
                {{ "superando los" | translate }} <span>{% if cart.free_shipping is defined and cart.free_shipping.min_price_free_shipping is defined %}{{ cart.free_shipping.min_price_free_shipping.min_price }}{% endif %}</span>
            </span>
        </span>
...
{# Product form, includes: Variants, CTA and Shipping calculator #}
 ...
 <form id="product_form" class="js-product-form" method="post" action="{{ store.cart_url }}" data-store="product-form-{{ product.id }}">
    ...
    {% set free_shipping_minimum_label_changes_visibility = has_free_shipping and cart.free_shipping is defined and cart.free_shipping.min_price_free_shipping is defined and cart.free_shipping.min_price_free_shipping.min_price_raw > 0 %}

    {% set include_product_free_shipping_min_wording = cart.free_shipping is defined and cart.free_shipping.min_price_free_shipping is defined and cart.free_shipping.min_price_free_shipping.min_price_raw > 0 %}
    ...
    {% if not product.is_non_shippable and show_product_quantity and has_free_shipping and not has_product_free_shipping %}
        ...
        <div class="js-product-form-free-shipping-message {% if free_shipping_minimum_label_changes_visibility %}js-free-shipping-message{% endif %} text-accent font-weight-bold mb-4 text-center text-md-left h6" {% if cart.free_shipping is not defined or not cart.free_shipping.cart_has_free_shipping %}style="display: none;"{% endif %}>
